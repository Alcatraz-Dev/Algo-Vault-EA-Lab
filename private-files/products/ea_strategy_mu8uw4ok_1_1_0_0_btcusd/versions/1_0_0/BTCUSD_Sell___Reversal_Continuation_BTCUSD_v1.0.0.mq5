...
#property description "Strategy: BTCUSD Sell — Reversal Continuation | ID: strategy_mu8uw4ok_1 | v1.0.0"
#property description "Generator: 1.0.0 | Hash: f6ad601a42aafcc1 | Magic: 605355"
...
// ─────────────────────────────────────────────────────────────────────
input string  EAStrategyId     = "strategy_mu8uw4ok_1";
input string  EAStrategyVersion = "1.0.0";
input string  EAConfigHash     = "f6ad601a42aafcc1";

//+------------------------------------------------------------------+
//| Inputs (defaults from the Strategy Lab strategy)                  |
//+------------------------------------------------------------------+
input ulong  MagicNumber        = 605355;   // Magic number (strategy/version/symbol)
input string OrderComment       = "AlgoVault BTCUSD";     // Order comment
input string TradeSymbol        = "BTCUSD";      // Trading symbol
input ENUM_TIMEFRAMES SetupTimeframe = PERIOD_M15; // Entry timeframe (attach to this chart TF)

input bool   EnableTrading      = true;                 // Enable trading
input int    MaxSlippagePoints  = 10; // Max deviation in points

input bool   UsePercentRisk     = true;
input double RiskPercent        = 1;
input double FixedLot           = 0.01;
input int    MaxPositions       = 1;
input int    MaxTradesPerDay    = 2;
input int    CooldownCandles    = 3;
input double DailyLossLimitPct  = 3;
input double MaxDrawdownPct     = 20;

input double TpR1               = 1;
input double TpR2               = 2;
input double TpR3               = 3;
input double TpFixedDistance    = 0;
input double SlAtrMultiple      = 1.5;
input double SlLevelOffset      = 0;
input double PartialClosePctR1  = 33;
input double PartialClosePctR2  = 33;
input bool   MoveBeAfterTp1     = true;
input bool   LockAfterTp2       = true;
input bool   EnableTrailing     = false;
input double TrailingStopAtr    = 1;

input double MaxSpreadPoints    = 2000; // 0 = off

//+------------------------------------------------------------------+
//| Trade management configuration (compiled from the strategy)       |
//+------------------------------------------------------------------+
input bool    MoveBEEnabled   = true;
input bool    LockEnabled     = true;
...
int OnInit()
{
   Print("AlgoVault EA: BTCUSD Sell — Reversal Continuation v1.0.0");
   Print("Strategy: BTCUSD Sell — Reversal Continuation | ID: strategy_mu8uw4ok_1 | Version: 1.0.0 | Hash: f6ad601a42aafcc1");
   Print("Generator: 1.0.0 | Magic: 605355 | Symbol: BTCUSD | Setup TF: M15");
   ...
   LoadFeatureState(f_H1, PERIOD_H1);
   LoadFeatureState(f_H4, PERIOD_H4);
   LoadFeatureState(f_M15, PERIOD_M15);
...
bool EntryConditionPassed()
{
   ...
   if(!((f_H4.trend == "bearish") && (f_H1.momentumPct < 0) && (f_H1.lowerHigh == true && f_H1.lowerLow == true))) return(false);
...
bool CanEnter()
{
   ...
   if(!StrIn(f_setup.session, "london,new_york"))
      return(false);

   // Day-of-week filter
   if(((1 << f_setup.weekdayUtc) & 62) == 0)
      return(false);
   ...
   // Regime filter
   {
      ...
      if(!StrIn(rg, "trending_bearish,breakout,high_volatility"))
         return(false);
   }
...
void TryEnter()
{
   ...
   double sl = entryPrice + SlAtrMultiple * atr;
   ...
   double tp1 = entryPrice - TpR1 * riskPrice;
   double tp2 = entryPrice - TpR2 * riskPrice;
   double tp3 = entryPrice - TpR3 * riskPrice;
   int digits = (int)SymbolInfoInteger(TradeSymbol, SYMBOL_DIGITS);
