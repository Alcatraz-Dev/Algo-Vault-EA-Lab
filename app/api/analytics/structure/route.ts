...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ success: true, symbol, timeframe, structure, candleCount: candles.length });
    } catch (err) {
        console.error("Structure API error:", err);
        return NextResponse.json({ error: "Failed to analyze structure" }, { status: 500 });
    }
}
