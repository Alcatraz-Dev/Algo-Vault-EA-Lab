...
export async function GET(request: NextRequest) {
    try {
        ...
        const orderList = Object.values(orders).map((o: any) => ({
            ticket: o.ticket, symbol: o.symbol, type: o.type, volume: o.volume,
            profit: o.profit || 0, status: o.status, slippage: o.slippageCost || 0,
            spreadCost: o.spreadCost || 0, commission: o.commission || 0,
        }));
        const filledOrders = orderList.filter((o: any) => o.status === "FILLED" || o.status === "closed").length;
        const rejectedOrders = orderList.filter((o: any) => o.status === "REJECTED" || o.status === "FAILED").length;
